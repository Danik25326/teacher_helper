...
// Підручник
$('textbook-upload').addEventListener('change', e => showTextbookPreviews(e.target.files));
...
// ── ПІДРУЧНИК ────────────────────────────────────────────────────
function showTextbookPreviews(files) {
...
async function loadTextbook() {
  ...
  if (!files.length) {
    showStatus('textbook-status', 'Спочатку виберіть файли підручника.', 'error');
    return;
  }
  showStatus('textbook-status', '⏳ Завантаження підручника…', 'loading');

  state.textbookPages = [];
  for (const f of files) {
    if (f.type.startsWith('image/')) {
      const b64 = await toBase64(f);
      state.textbookPages.push(b64);
    }
  }

  if (!state.textbookPages.length) {
    showStatus('textbook-status', '⚠️ Підтримуються лише зображення (PNG/JPG). PDF поки не підтримується браузером напряму.', 'error');
    return;
  }

  state.textbookLoaded = true;
  showStatus('textbook-status', `✅ Підручник завантажено: ${state.textbookPages.length} сторінок. ШІ буде використовувати їх при перевірці.`, 'success');
}
...
async function checkWork() {
  ...
  try {
    ...
    const sheetImages = [];
    for (const f of sheets) sheetImages.push(await toBase64(f));
    ...
    // Додаємо сторінки підручника якщо є
    if (state.textbookPages.length > 0) {
      contentBlocks.push({ type: 'text', text: `Ось сторінки підручника для контексту (${state.textbookPages.length} стор.):` });
      state.textbookPages.slice(0, 5).forEach(b64 => { // max 5 стор підручника
        contentBlocks.push({ type: 'image_url', image_url: { url: `data:image/jpeg;base64,${b64}` } });
      });
    }
